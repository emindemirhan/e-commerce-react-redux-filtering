    ...
    <li className={styles.productsBodyListItem}>
      <NavLink to={`/detail/${id}`}>
        ...
        <div className={styles.productsBodyListItemBody}>
          <div>
            <Text tag="h3" fontSize="" fontWeight="semibold">
              {title}
            </Text>
          </div>
...
const ProductList = () => {
  const { filteredProducts, products, productsLoading } = useSelector(
    (state: any) => state
  );
  ...
  return (
    <>
      {!productsLoading ? (
        products.length === 0 ? (
          <Alert>No product found as a result of the search.</Alert>
        ) : products.length > 0 && filteredProducts.length === 0 ? (
          <Alert>No filtered product found.</Alert>
        ) : (
          <ul className={styles.productsBodyList}>
            {filteredProducts
              ?.slice(0, showLimit)
              .map((item: any) => <Item key={item.id} obj={item} />)}

            {filteredProducts.length > showLimit && (
              <div className={styles.productsBodyListShowMore}>
                {" "}
                <Button
                  className={[styles.productsBodyListShowMoreBtn]}
                  onClick={() => setShowLimit((prevState) => (prevState += 15))}
                >
                  Show More
                </Button>{" "}
              </div>
